...
class CCBPress_Widget_Login extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		ob_start();
		?>
		<form action="<?php esc_attr_e( $ccb_login_url ); ?>" method="post" target="_blank">
			<input type="hidden" name="ax" value="login" />
			<input type="hidden" name="rurl" value="" />
			<label for="username_<?php esc_attr_e( $args['widget_id'] ); ?>"><?php _e('Username:', 'ccbpress-core'); ?></label>
			<input id="username_<?php esc_attr_e( $args['widget_id'] ); ?>" type="text" name="form[login]" value="" />
			<label for="password_<?php esc_attr_e( $args['widget_id'] ); ?>"><?php _e('Password:', 'ccbpress-core'); ?></label>
			<input id="password_<?php esc_attr_e( $args['widget_id'] ); ?>" type="password" name="form[password]" value="" />
			<p>
				<input type="submit" value="<?php esc_attr_e( __('Login', 'ccbpress-core') ); ?>" />
			</p>
		</form>
		<p>
			<a href="<?php esc_attr_e( $ccb_password_url ); ?>" target="_blank"><?php _e('Forgot username or password?', 'ccbpress-core'); ?></a>
		</p>
		<?php
		echo ob_get_clean();
