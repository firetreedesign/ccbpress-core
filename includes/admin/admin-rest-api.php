...
class CCBPress_Admin_REST_API {
	...
	public static function rest_api_init() {
		...
		register_rest_route( 'ccbpress/v1', '/admin/purge_image_cache', array(
	        'methods' => 'POST',
			'callback' => __CLASS__ . '::purge_image_cache',
        ) );
        
        register_rest_route( 'ccbpress/v1', '/admin/purge_transient_cache', array(
            'methods' => 'POST',
            'callback' => __CLASS__ . '::purge_transient_cache',
        ) );

        register_rest_route( 'ccbpress/v1', '/admin/check_api_services', array(
            'methods' => 'POST',
            'callback' => __CLASS__ . '::check_api_services',
        ) );

        register_rest_route( 'ccbpress/v1', '/admin/reset_import', array(
            'methods' => 'POST',
            'callback' => __CLASS__ . '::reset_import',
        ) );

        register_rest_route( 'ccbpress/v1', '/admin/start_import', array(
            'methods' => 'POST',
            'callback' => __CLASS__ . '::start_import',
        ) );

        register_rest_route( 'ccbpress/v1', '/admin/last_import', array(
            'methods' => 'POST',
            'callback' => __CLASS__ . '::last_import',
        ) );

        register_rest_route( 'ccbpress/v1', '/admin/import_status', array(
            'methods' => 'POST',
    ...
    public static function check_api_services( WP_REST_Request $request ) {
        ...
        foreach ( $services as $service ) {
            ...
            if ( true === $service_result[0] ) {
                $results[] = array( 'service' => $service, 'status' => esc_html__( 'Passed', 'ccbpress-core' ) );
            } else {
                if ( isset( $ccb_data->response->errors->error ) ) {
                    $results[] = array( 'service' => $service, 'status' => (string) $ccb_data->response->errors->error );
                } else {
                    $results[] = array( 'service' => $service, 'status' => esc_html__( 'There was an error with this service', 'ccbpress-core' ) );
                }
            }
    ...
    public static function last_import( WP_REST_Request $request ) {
		...
		if ( 'Never' === $last_import ) {
			...
			$response = esc_html( human_time_diff( strtotime( 'now', current_time( 'timestamp' ) ), strtotime( $last_import, current_time( 'timestamp' ) ) ) . ' ago' );
        }
