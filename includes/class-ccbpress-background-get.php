	...
	protected function task( $item ) {
		...
		if ( ! is_null( $item['addon'] ) ) {
			if ( isset( $item['query_string']['page'] ) && isset( $item['query_string']['per_page'] ) ) {
				update_option( 'ccbpress_' . $item['addon'] . '_import_in_progress', 'Processing ' . $item['per_page'] . ' records ' . $item['query_string']['srv'] . ' at page ' . $item['page'] . '...' );
			} else {
				update_option( 'ccbpress_' . $item['addon'] . '_import_in_progress', 'Processing ' . $item['query_string']['srv'] . '...' );
			}
		}

		$response = CCBPress()->ccb->get_new( $item );
		$srv = strtolower( $item['query_string']['srv'] );
		$response = apply_filters( "ccbpress_background_get_{$srv}", $response, $args );
